# ==============================================================================
# 自定义天气段 (Custom Weather Segment for Powerlevel10k)
# 功能: 显示当前天气，并找出明天午夜前下一次恶劣天气的开始时间、持续时长和天气类型汇总。
#       使用 P10K_WEATHER_LAT/LON 环境变量和纯文本。
# ==============================================================================

# 辅助函数：将 weatherCode 转换为具体的纯文本描述
_weather_code_to_text() {
  local code=$1
  case "$code" in
    "1000") echo "Clear, Sunny";;
    "1001") echo "Cloudy";;
    "1100") echo "Mostly Clear";;
    "1101") echo "Partly Cloudy";;
    "1102") echo "Mostly Cloudy";;
    "2000") echo "Fog";;
    "2100") echo "Light Fog";;
    "4000") echo "Drizzle";;
    "4001") echo "Rain";;
    "4200") echo "Light Rain";;
    "4201") echo "Heavy Rain";;
    "5000") echo "Snow";;
    "5001") echo "Flurries";;
    "5100") echo "Light Snow";;
    "5101") echo "Heavy Snow";;
    "6000") echo "Freezing Drizzle";;
    "6001") echo "Freezing Rain";;
    "6200") echo "Light Freezing Rain";;
    "6201") echo "Heavy Freezing Rain";;
    "7000") echo "Ice Pellets";;
    "7101") echo "Heavy Ice Pellets";;
    "7102") echo "Light Ice Pellets";;
    "8000") echo "Thunderstorm";;
    *)      echo "Unknown";;
  esac
}

# 辅助函数：判断 weatherCode 是否为“坏天气”（任何降水或雷暴）
_is_bad_weather_code() {
  local code=$1
  case "$code" in
    4* | 5* | 6* | 7* | 8*) return 0 ;; # true
    *) return 1 ;; # false
  esac
}

# 辅助函数：根据天气严重性返回颜色代码
_get_weather_severity_color() {
    local code=$1
    case "$code" in
        # 红色 (1): 中到重度恶劣天气
        "4001"|"4201"|"5000"|"5101"|"6001"|"6201"|"7000"|"7101"|"8000")
            echo 1 ;;
        # 黄色 (3): 轻微恶劣天气
        "4000"|"4200"|"5100"|"6000"|"6200"|"7102")
            echo 3 ;;
        # 绿色 (2): 非恶劣天气
        *)
            echo 2 ;;
...
# 辅助函数：将 weatherCode 转换为通用的天气类型（用于汇总）
_weather_code_to_general_type() {
    local code=$1
    case "$code" in
        4*) echo "󰖗" ;;
        5*) echo "󰖘" ;;
...
# Powerlevel10k 的主函数 (高级文本预警版)
function prompt_weather() {
  # 依赖项和配置检查
  if ! command -v curl &>/dev/null || ! command -v jq &>/dev/null; then p10k segment -f red -t 'weather: curl/jq missing'; return; fi
  if [[ -z "$TOMORROWIO_API_KEY" ]]; then p10k segment -f red -t 'TOMORROWIO_API_KEY not set'; return; fi
  local lat=${P10K_WEATHER_LAT:-"31.23"}; local lon=${P10K_WEATHER_LON:-"121.47"}; local api_key=$TOMORROWIO_API_KEY

  # 缓存处理
  local cache_file="/tmp/p10k_weather_cache_${lat}_${lon}.json"
  local weather_data=""; if [[ -f "$cache_file" ]] && ! find "$cache_file" -mmin +29 -print | grep -q .; then weather_data=$(<"$cache_file"); fi

  # 获取新数据
  if [[ -z "$weather_data" ]]; then
    local api_url="https://api.tomorrow.io/v4/weather/forecast?location=${lat},${lon}&apikey=${api_key}"
    local api_response=$(curl --silent --fail --max-time 2 --request GET --url "$api_url")
    if [[ $? -ne 0 || -z "$api_response" ]]; then p10k segment -f yellow -t "NetErr"; return; fi
    weather_data="$api_response"; rm -f /tmp/p10k_weather_cache_*; echo "$weather_data" > "$cache_file"
  fi

  # 解析数据 local current_conditions=$(echo "$weather_data" | jq -e -r '.timelines.hourly[0].values | "\(.temperature) \(.weatherCode)"')
  local hourly_forecast=$(echo "$weather_data" | jq -e -r '.timelines.hourly[] | "\(.time) \(.values.weatherCode)"')
  if [[ $? -ne 0 || -z "$current_conditions" || -z "$hourly_forecast" ]]; then p10k segment -f red -t "ParseErr"; rm -f "$cache_file"; return; fi

  # --- 核心逻辑：查找并汇总恶劣天气事件 ---
  local event_start_time=""; local event_duration=0; typeset -A event_weather_types; local in_bad_weather_event=false
  local eod_tomorrow_ts; if [[ "$(uname)" == "Darwin" ]]; then eod_tomorrow_ts=$(date -v+1d -v23H -v59M -v59S +%s); else eod_tomorrow_ts=$(date -d "tomorrow 23:59:59" +%s); fi

  echo "$hourly_forecast" | while read -r time_utc code; do
    local forecast_ts; if [[ "$(uname)" == "Darwin" ]]; then forecast_ts=$(date -jf "%Y-%m-%dT%H:%M:%SZ" +"%s" "$time_utc"); else forecast_ts=$(date -d "$time_utc" +"%s"); fi
    if (( forecast_ts > eod_tomorrow_ts )); then break; fi
    if _is_bad_weather_code "$code"; then
      if ! $in_bad_weather_event; then in_bad_weather_event=true; event_start_time=$time_utc; fi
      ((event_duration++)); local general_type=$(_weather_code_to_general_type "$code"); event_weather_types[$general_type]=1
    else
      if $in_bad_weather_event; then break; fi
    fi
  done

  # --- 格式化并显示结果 ---
  # 段 1: 当前温度 (固定颜色)
  local temp current_code; read -r temp current_code <<< "$current_conditions"
  local temp_int=$(printf "%.0f" "$temp")
  p10k segment -f 7 -t "${temp_int}°C"

  # 段 2: 当前天气 (动态颜色)
  local current_text=$(_weather_code_to_text "$current_code")
  local current_color=$(_get_weather_severity_color "$current_code")
  local current_weather_string="${current_text}"
  p10k segment -f $current_color -t " $current_weather_string"

  # 段 3: 未来预警 (固定颜色)
  local forecast_string=""
  if [[ -n "$event_start_time" ]]; then
    local formatted_time; local relative_day; local today_str=$(date +"%Y-%m-%d"); local event_date_str
    if [[ "$(uname)" == "Darwin" ]]; then formatted_time=$(date -jf "%Y-%m-%dT%H:%M:%SZ" +"%H:%M" "$event_start_time"); event_date_str=$(date -jf "%Y-%m-%dT%H:%M:%SZ" +"%Y-%m-%d" "$event_start_time"); else formatted_time=$(date -d "$event_start_time" +"%H:%M"); event_date_str=$(date -d "$event_start_time" +"%Y-%m-%d"); fi
    local tomorrow_str=$(date -d "$today_str + 1 day" +"%Y-%m-%d")
    if [[ "$event_date_str" == "$today_str" ]]; then relative_day="Today"; elif [[ "$event_date_str" == "$tomorrow_str" ]]; then relative_day="Tomorrow"; else relative_day=$(date -d "$event_date_str" +"%a"); fi
    local weather_summary=$(echo ${(k)event_weather_types} | tr ' ' '+')
    forecast_string=" ${weather_summary} ${relative_day} ${formatted_time} ~${event_duration}h"
  else
    forecast_string=" Clear until Tomorrow Night"
  fi
  p10k segment -f 5 -t "$forecast_string"
}

# ==============================================================================

# Generated by Powerlevel10k configuration wizard on 2025-05-27 at 11:15 CST.
...
# 24h time, 2 lines, dotted, no frame, green-ornaments, sparse, many icons, concise,
