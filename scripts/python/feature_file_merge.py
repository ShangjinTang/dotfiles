...
class FeatureFileMerger:
    ...
    def generate(self):
        ...
        for file in self.input_files:
            ...
            logging.debug("Features read from input file {}: {}".format(
                file, file_features))
            self.features.update(file_features)
        logging.debug("Features write to output file {}: {}".format(
            self.output_file, self.features))
            ...
            with open(file, 'r') as f:
                ...
                for line in f.readlines():
                    ...
                    if line and not line.startswith(self.comment_symbol):
                        ...
                        if len(line_split) != 2:
                            logging.warning(
                                "file: {} line {}: '{}' is not valid, ignore it"
                                .format(file, line_number, line))
                            continue
                        key, value = line_split[0].strip(
                        ), line_split[1].strip()
                        features[key] = value
        return features
...
def main(args):
    try:
        ...
        feature_file_merger = FeatureFileMerger(args.input, args.output,
                                                args.priority, file_utils)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--comment_symbol',
                        default="#",
                        help="Comment symbol (default #)")
    parser.add_argument('--separate_symbol',
                        default="=",
                        help="Separate symbol (default =)")
